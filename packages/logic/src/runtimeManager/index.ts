...
class RuntimManager {
  ...
  pageHide(opts: { id: any; }) {
    const { id } = opts;
    const page = this.pages[id];
    (page as any)?.onHide?.();
  }

  pageReady(opts: { id: any; }) {
    const { id } = opts;
    const page = this.pages[id];
    (page as any)?.onReady?.();
  }

  pageScroll(opts: { id: any; scrollTop: any; }) {
  ...
  // 调用小程序自定义逻辑方法: 主要是页面出发逻辑事件
  triggerEvent(opts: { id: any; methodName: any; paramsList: any; }) {
